...
fn qwen3_matches_hf_golden() {
    ...
    let golden: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(repo_path("golden/qwen3-golden.json")).unwrap(),
    )
    .unwrap();
    ...
    let gen =
        pollster::block_on(eyebrowse::Generator::load(model_dir.to_str().unwrap(), 256)).unwrap();
    ...
    assert_eq!(
        out[0], top1,
        "first-token argmax mismatch: got {}, want {}",
        out[0], top1
    );
